...
enum imagetype_t { it_skin, it_sprite, it_wall, it_pic, it_sky };

struct image_t {
	image_t( const std::string &name, unsigned int w, unsigned int h, imagetype_t type ) : 
		name( name ), 
		type( type ), 
		width( w ), height( h ),
		upload_width( w ), upload_height( h )
	{}

	std::string name;
	imagetype_t type;

	unsigned int width, height;                // source image
	int upload_width, upload_height;  // after power of two and picmip

	struct msurface_s *texturechain{ nullptr };  // for sort-by-texture world drawing

	bgfx::TextureHandle handle{ bgfx::kInvalidHandle }; // texture handle

	float sl{ 0.0f }, tl{ 0.0f }, sh{ 1.0f }, th{ 1.0f };             // 0,0 - 1,1 unless part of the scrap
