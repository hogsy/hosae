...
static std::map< std::string, hosae::Model > cachedModels;
...
hosae::Model *Mod_ForName( const char *name, qboolean crash )
{
	...
	{
		int i = atoi( name + 1 );
		if ( i < 1 || r_worldmodel == nullptr || i >= r_worldmodel->subModels.size() )
			VID_Error( ERR_DROP, "bad inline model number" );

		return r_worldmodel->subModels[ i ];
	}

	// search the currently loaded models
	auto i = cachedModels.find( name );
	if ( i != cachedModels.end() )
		return &i->second;

	// load the file
	int modfilelen = FS_LoadFile( name, ( void ** ) &buf );
	if ( buf == nullptr )
	{
		if ( crash )
			VID_Error( ERR_DROP, "Mod_NumForName: %s not found", name );
	...
	{
		case IDALIASHEADER:
			model = new hosae::MD2Model( buf );
			break;
		case IDSPRITEHEADER:
			model = new hosae::SpriteModel( buf );
			break;
		case IDBSPHEADER:
			model = new hosae::BSPModel( buf );
			break;

		default:
#if defined( _DEBUG )
			VID_Printf( PRINT_ALL, "Mod_NumForName: unknown fileid for %s", name );
#else
			VID_Error( ERR_DROP, "Mod_NumForName: unknown fileid for %s", name );
#endif
			break;
	}
...
hosae::BSPModel::BSPModel( void *buffer )
{
	...
	LoadVertices( &header->lumps[ LUMP_VERTEXES ] );
	...
	LoadTextureInfo( &header->lumps[ LUMP_TEXINFO ] );
	...
	for ( int i = 0; i < mod->numsubmodels; i++ )
	{
		...
		hosae::BSPModel *starmod = subModels[ i ];
...
hosae::MD2Model::MD2Model( void *buffer )
{
	...
	for ( int i = 0; i < pheader->num_frames; i++ )
	{
		switch ( pheader->resolution )
		{
			case 0:
			{
				...
				for ( int32_t j = 0; j < pheader->num_xyz; ++j )
				{
	...
	// load the glcmds
	int32_t *pincmd  = ( int32_t * ) ( ( byte * ) pinmodel + pheader->ofs_glcmds );
	int32_t *poutcmd = ( int32_t * ) ( ( byte * ) pheader + pheader->ofs_glcmds );
	...
	for ( int32_t i = 0; i < pheader->num_skins; i++ )
	{
		...
		skins[ i ] = GL_FindImage( skin_path, it_skin );
...
=================
*/
hosae::SpriteModel::SpriteModel( void *buffer )
{
	...
	if ( sprout->version != SPRITE_VERSION )
		VID_Error( ERR_DROP, "%s has wrong version number (%i should be %i)", GetName(), sprout->version, SPRITE_VERSION );

	if ( sprout->numframes > MAX_MD2SKINS )
		VID_Error( ERR_DROP, "%s has too many frames (%i > %i)", GetName(), sprout->numframes, MAX_MD2SKINS );
	...
	for ( i = 0; i < sprout->numframes; i++ )
	{
		...
		skins[ i ] = GL_FindImage( sprout->frames[ i ].name, it_sprite );
	}
...
void Mod_BeginRegistration( const char *model )
{
	...
	r_worldmodel = static_cast< hosae::BSPModel *>( Mod_ForName( fullname, true ) );
	r_viewcluster = -1;
}

hosae::Model *Mod_RegisterModel( const char *name )
{
	hosae::Model *mod = Mod_ForName( name, false );
	if ( mod == nullptr )
		return nullptr;
	...
	if ( mod->GetType() == hosae::ModelType::SPRITE )
	{
		hosae::SpriteModel *bspModel = static_cast< hosae::BSPModel * >( mod );

		dsprite_t *sprout = ( dsprite_t * ) mod->extradata;
		for ( int i = 0; i < sprout->numframes; i++ )
			mod->skins[ i ] = GL_FindImage( sprout->frames[ i ].name, it_sprite );
	}
	else if ( mod->GetType() == hosae::ModelType::BSP )
	{
		hosae::BSPModel *bspModel = static_cast< hosae::BSPModel * >( mod );
		for ( int i = 0; i < bspModel->numtexinfo; i++ )
			bspModel->texinfo[ i ].image->registration_sequence = registration_sequence;
	}
